...
# ==============================================================================
# 全自动 VMess + WS + TLS 节点部署脚本 (最终稳定版)
#
# 更新: 1. 采用向Python传递参数的方式，彻底解决Bash解析冲突问题。
...
# !! 本次修正: 使用 'EOF' 防止Bash解析，并通过参数传递变量 !!
...
def generate_vmess_config(full_domain, provider, country_name_cn):
    profile_name = f"{provider}-{country_name_cn}"
    vmess_config = {"v": "2", "ps": profile_name, "add": full_domain, "port": "443", "id": str(uuid.uuid4()), "aid": 0, "net": "ws", "type": "none", "host": full_domain, "path": "/v2", "tls": "tls"}
    import base64
...
def write_xray_config(vmess_config):
    ...
    server_config = {
      ...
      "inbounds": [{"port": 443, "protocol": "vmess", "settings": {"clients": [{"id": vmess_config['id'], "alterId": 0}]}, "streamSettings": {"network": "ws", "security": "tls", "tlsSettings": {"certificates": [{"certificateFile": FULLCHAIN_PATH, "keyFile": KEY_PATH}]}, "wsSettings": {"path": vmess_config['path']}}}],
...
def main():
    ...
    vmess_config, vmess_link = generate_vmess_config(full_domain, provider, country_name_cn)
