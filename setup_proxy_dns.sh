...
# ==============================================================================
# 全自动 VMess + WS + TLS 节点部署脚本 (集成指定DNS最终版 - 修复IP获取 + acme路径检测)
# ==============================================================================
...
# --- 步骤 3: 检测并安装 acme.sh ---
if [ ! -x "/root/.acme.sh/acme.sh" ] && ! command -v acme.sh >/dev/null 2>&1; then
    echo "[*] acme.sh 未安装，正在为您自动安装..."
    # 不再静音，方便排错
    curl -s https://get.acme.sh | sh
    echo "[*] acme.sh 安装命令执行完毕。"
else
    echo "[*] acme.sh 已安装，跳过安装步骤。"
fi
echo

# --- 步骤 3.1: 确认 acme.sh 实际路径，并用于后续 Python 调用 ---
ACME_BIN="$(command -v acme.sh 2>/dev/null)"
if [[ -z "$ACME_BIN" && -x "/root/.acme.sh/acme.sh" ]]; then
    ACME_BIN="/root/.acme.sh/acme.sh"
fi

if [[ -z "$ACME_BIN" ]]; then
    echo "[x] acme.sh 安装失败或路径不可识别，请手动执行以下命令检查："
    echo "    ls -R /root/.acme.sh"
    echo "    command -v acme.sh"
    exit 1
fi

echo "[√] acme.sh 检测成功，路径: $ACME_BIN"
echo
...
python3 - "$CLOUDFLARE_EMAIL" "$GLOBAL_KEY" "$DOMAIN" "$TG_BOT_TOKEN" "$TG_CHAT_ID" "$ACME_BIN" <<'EOF'
import random
import string
import requests
import json
...
# 从命令行参数读取变量
if len(sys.argv) < 7:
    print("[x] 错误: 脚本参数不足。需要: CLOUDFLARE_EMAIL GLOBAL_KEY DOMAIN TG_BOT_TOKEN TG_CHAT_ID ACME_PATH")
    sys.exit(1)
...
def get_ip_info():
    ...
    try:
        ...
        response = requests.get(
            "http://ip-api.com/json/?fields=status,country,countryCode,isp,org",
            timeout=5
        ).json()
        if response.get("status") == "success":
            ...
            provider = "VPS"
            if "microsoft" in org_name:
                provider = "AZ"
            elif "amazon" in org_name:
                provider = "AWS"
            elif "google" in org_name:
                provider = "GCP"
            elif "oracle" in org_name:
                provider = "OCI"
            elif "vultr" in org_name:
                provider = "Vultr"
            elif "digitalocean" in org_name:
                provider = "DO"
            country_map = {
                "South Korea": "韩国", "Japan": "日本", "Hong Kong": "香港",
                "Taiwan": "台湾", "Singapore": "新加坡", "United States": "美国",
                "United Kingdom": "英国", "Germany": "德国", "France": "法国",
                "Netherlands": "荷兰", "Russia": "俄罗斯", "Canada": "加拿大",
                "Australia": "澳大利亚", "Malaysia": "马来西亚", "Thailand": "泰国"
            }
...
def install_certificate(full_domain):
    if not ACME_PATH or not os.path.isfile(ACME_PATH):
        safe_print(f"[x] 错误: acme.sh 未找到或不可执行，路径: {ACME_PATH}")
        sys.exit(1)
    ...
    command = [
        ...
        "--dns", "dns_cf",
        "-d", full_domain,
        "--key-file", TEMP_KEY_PATH,
        "--fullchain-file", TEMP_FULLCHAIN_PATH,
        "--force"
    ]
    ...
    if process.returncode != 0:
        safe_print(f"[x] 错误: 证书申请失败，acme.sh 返回码: {process.returncode}")
        sys.exit(1)
    safe_print(f"[√] 证书已成功申请并保存到 {TEMP_FULLCHAIN_PATH} / {TEMP_KEY_PATH} !")
...
def generate_vmess_config(full_domain, provider, country_name_cn, port):
    ...
    vmess_link = "vmess://" + base64.b64encode(
        json.dumps(vmess_config, separators=(',', ':')).encode('utf-8')
    ).decode('utf-8')
...
XRAY_GROUP=$(id -gn "$XRAY_USER" 2>/dev/null)
